...
Collects GPU hardware counters (OA metrics) and maps bottlenecks to KB
...
def print_recommendations(
    ...
    # --- Host overhead check ---
    # Grounded in: kb/memory_patterns.yaml (no_device_to_host_scalar_sync)
    ...
    if total_host_time > 0 and total_gpu_time > 0 and total_host_time > total_gpu_time * 2:
        recommendations.append(
            (
                ...
                "kb/memory_patterns.yaml (no_device_to_host_scalar_sync)",
            )
        )

    # --- Overhead kernel dominance ---
    # Grounded in: kb/optimization_levels.yaml (level_2_bandwidth_reduction)
    ...
    if has_oa:
        ...
        # Grounded in: kb/optimization_levels.yaml (level_2_bandwidth_reduction)
        # and kb/xpu_optimizations.yaml (xpu_descriptor_gemm_pattern)
        ...
        if stalled is not None and active is not None and stalled > active:
            recommendations.append(
                (
                    ...
                    "kb/xpu_optimizations.yaml (xpu_descriptor_gemm_pattern, xpu_tile_swizzling) + "
                    "kb/optimization_levels.yaml (level_2_bandwidth_reduction)",
                )
            )

        # --- XVE Idle-dominated: underutilization ---
        # Grounded in: kb/persistent_kernel_patterns.yaml (persistent_kernel_basic_tile_loop)
        ...
        if idle is not None and active is not None and idle > 30 and active < 30:
            recommendations.append(
                (
                    ...
                    "kb/persistent_kernel_patterns.yaml (persistent_kernel_basic_tile_loop)",
                )
            )

        # --- Occupancy limiters ---
        # Grounded in: kb/xpu_optimizations.yaml (xpu_grf_mode, xpu_tile_swizzling)
            ...
            if (
                ...
                recommendations.append(
                    (
                        ...
                        "kb/xpu_optimizations.yaml (xpu_tile_swizzling) + "
                ...
                recommendations.append(
                    (
                        ...
                        "kb/xpu_optimizations.yaml (xpu_grf_mode)",
                    )
                )
                ...
                recommendations.append(
                    (
                        ...
                        "kb/xpu_optimizations.yaml (xpu_warp_count)",
                    )
                )
        ...
        # Grounded in: kb/xpu_optimizations.yaml (xpu_descriptor_gemm_pattern, xpu_tile_swizzling)
        ...
        # Grounded in: kb/xpu_optimizations.yaml (xpu_descriptor_gemm_pattern)
        # and kb/memory_patterns.yaml (mem_block_pointers)
        if lsc_l3_miss is not None and lsc_l3_miss > 50:
            recommendations.append(
                (
                    ...
                    "kb/xpu_optimizations.yaml (xpu_descriptor_gemm_pattern) + "
                    "kb/memory_patterns.yaml (mem_block_pointers)",
                )
            )

        # --- Register spill ---
        # Grounded in: kb/memory_patterns.yaml (reduce_liveness_sink_load_and_prefetch)
        # and kb/xpu_optimizations.yaml (xpu_grf_mode)
        ...
        if spill is not None and spill > 0:
            recommendations.append(
                (
                    ...
                    "kb/memory_patterns.yaml (reduce_liveness_sink_load_and_prefetch) + "
                    "kb/xpu_optimizations.yaml (xpu_grf_mode)",
                )
            )
        ...
        if occupancy is not None and occupancy < 50:
            recommendations.append(
                (
                    ...
                    "kb/xpu_optimizations.yaml (xpu_grf_mode, xpu_tile_swizzling)",
                )
            )
